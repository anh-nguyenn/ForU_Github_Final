...
struct InternalRotationWithResistanceBandRepetitionClassifier: Classifier {
    static func check(observation: VNHumanBodyPoseObservation) -> Bool {
        ...
        // check if arm is rotating inwards
        if armToOpShoulderAngle + 15 > internalRotationWithResistanceBandModel.lastAngle {
            return false
        }
