...
struct ExternalRotationWithResistanceBandRepetitionInProgressClassifier: Classifier {
    static func check(observation: VNHumanBodyPoseObservation) -> Bool {
        ...
        if armToOpShoulderAngle >= externalRotationWithResistanceBandModel.lastAngle - externalRotationWithResistanceBandModel.leeway && armToOpShoulderAngle <= externalRotationWithResistanceBandModel.lastAngle + externalRotationWithResistanceBandModel.leeway {
            externalRotationWithResistanceBandModel.currentAngleFrames += 1
            return true
        } else {
            externalRotationWithResistanceBandModel.currentAngleFrames = 0
            
            // Check that arms is rotating outwards
            if armToOpShoulderAngle < externalRotationWithResistanceBandModel.lastAngle - 3 {
                externalRotationWithResistanceBandModel.repetitionIsGood = false
                return false
            }
        }
        if armToOpShoulderAngle >= externalRotationWithResistanceBandModel.lastAngle - 2 || armToOpShoulderAngle <= externalRotationWithResistanceBandModel.lastAngle + 2 {
            externalRotationWithResistanceBandModel.lastAngle = armToOpShoulderAngle
        }
        
        externalRotationWithResistanceBandModel.repetitionIsGood = true
        return true
    }
}
